...
const getBalances = async (address) => {
  const balanceRequests = Object.keys(contractAddresses).map(token => getBalance(token, address));
  const results = await Promise.all(balanceRequests);
  const balances = results.reduce((acc, result) => Object.assign(acc, result), {});
  return balances;
};


const myAddress = '0x230c2c12d2d3e3965eacfea3a7b56efeaa823c18'
getBalances(myAddress).then(console.log).catch(console.error);
